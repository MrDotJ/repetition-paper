...
class baseBlock
    ...
    def optimize_model(self, f)
        self.model.optimize()
        f.write('nobj ' + str(self.model.getAttr('ObjVal')) + 'n')
        load_value = []
        for i in range(self.cycle)
            load_value.append(self.power[i].getAttr('X'))
        # f.write('load value n')
        # f.write('n'.join(str(load_value)))
        # f.write('n')
        result = []
...
def factory()
    build_1_basic_info = {
        ...
        'grid_buy_price' 0.25,
        'grid_sell_price' 0.1,  # change we can invest the response-demand
        'node_count' 5,
        'connection' [1],
    ...
    build_2_gener_info = {
        'PV_output' [2.0, 4.0, 5.5, 6.0, 7.0, 5.0, 4.0, 3.5, 3.0],
        'chp_MIN' 10,
        'chp_MAX' 500,
        'gas_price' 0.15,
        'chp_convert_to_power' 1
...
def start_go()
 ...
 #   '''
    ...
    while iterator_count  8
        f.write('nnniterator' + str(iterator_count) + 'n')
        ...
        f.write('n exchange n')
        f.write(n.join(str(item) for item in g_exchange))
        # S2                                # update aux
        aux = update_aux_variable(g_exchange, dual, tao, cycle, node_count, connection_info)
        # S3                                # update dual
        update_dual_variable(dual, tao, aux, g_exchange, cycle, node_count)

        f.write('n aux n')
        f.write(n.join(str(item) for item in aux))
        f.write('n dual n')
        f.write(n.join(str(item) for item in dual))
        f.write('n')
        normal = 0
