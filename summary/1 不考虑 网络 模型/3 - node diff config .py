...
# success! player2 and player3 have different config
...
#                  [0 1 3]
#                  [0 1 2]]
price555 = [[0, 5, 8.5],       # you tao li , 3 mai 2 mai 1
         [5, 0, 5],
         [5, 5, 0]]   # 你要注意这都是买的价格
price = [[0, 8.5, 8.5],
         [8.5, 0, 8.5],
         [8.5, 8.5, 0]]   # 你要注意这都是买的价格
g_tao = 10
g_ex = [[0, 0],
        [0, 0],
        [0, 0]]
g_lam = [0, 0, 0, 0, 0, 0]  # 01 01 02 02 12 12
g_lam_index = [[0, 2],
               [0, 4],
               [2, 4]]
g_connection = [[1, 2],
...
def start():
    ...
    while outer_loop_count < 200:
        ...
        result_plt3.append(g_ex[0][1])
        result_plt4.append(g_ex[2][0])
        result_plt5.append(g_ex[1][1])
        result_plt6.append(g_ex[2][1])
        if ((abs(g_ex[0][0] + g_ex[1][0]) < 0.1) &
                (abs(g_ex[0][1] + g_ex[2][0]) < 0.1) &
                 (abs(g_ex[1][1] + g_ex[2][1]) < 0.1)):
            print(outer_loop_count)   # 67
            break
    ...
    plt.plot(result_plt3, label='0->2')
    plt.plot(result_plt4, label='2->0')
    plt.plot(result_plt5, label='1->2')
    plt.plot(result_plt6, label='2->1')
    plt.legend(loc='best')
    plt.savefig('3-node-elec-diff-config.svg')
