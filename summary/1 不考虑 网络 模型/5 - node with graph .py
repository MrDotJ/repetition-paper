...
price = [[0, 6, 6, 6, 6],
         ...
         [6, 6, 0, 6, 6],
         [6, 6, 6, 0, 6],
         [6, 6, 6, 6, 0]]   # 你要注意这都是买的价格
#price = [[6]*5]*5   # 你要注意这都是买的价格
g_tao = 10
g_ex = [[0],
        [0, 0, 0],
        [0, 0],
...
def calculate_NE():
    ...
    while count_best_response < 20:
        ...
        for i, player in enumerate(g_players):
            ...
            # update the response
        if sub(g_ex_old , copy.deepcopy(g_ex)) < 0.0001:
...
def draw_flow():
    ...
    # dot.edges(['12', '23', '24', '35'])
    dot.edge('1' if g_ex[0][0] > 0 else '2', '2' if g_ex[0][0] > 0 else '1' , str(abs(g_ex[0][0]))[0:5])
    dot.edge('2', '3', str(g_ex[1][1])[0:5])
    dot.edge('2', '4', str(g_ex[1][2])[0:5])
    dot.edge('3', '5', str(g_ex[2][1])[0:5])
    # 在创建两圆点之间创建一条边
    # dot.edge('B', 'C', 'test')
...
def start():
    ...
    while outer_loop_count < 200:
        print('outer_loop_count: ' + str(outer_loop_count))
        ...
        if ((abs(g_ex[0][0] + g_ex[1][0]) < 0.1) &
                (abs(g_ex[2][0] + g_ex[1][1]) < 0.1) &
                (abs(g_ex[3][0] + g_ex[1][2]) < 0.1) &
                (abs(g_ex[2][1] + g_ex[4][0]) < 0.1)):
            print(outer_loop_count)     #193
            break;
