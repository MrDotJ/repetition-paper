...
class GasNet:
    ...
    def build_base(self):
        ...
        for node in range(self.gas_node_num):
            ...
            for time in range(self.T):
                self.model.addConstr(
                    ...
                    rhs=sum(Gen[:, time]) + 0.01 * sum(Load[:, time]) + sum(Line_Out[:, time]),  # load
        ...
        self.model.setObjective(gurobi.quicksum(self.well_output))
...
def get_load_min_max():
    a = [5520, 4920, 4680, 4740, 5100, 5640, 5580, 6060, 6180]
    load_base = np.array(a) * 28.3
    min_coeff = 0.8
    max_coeff = 1.2
...
def build_gas_network():
    ...
    gas_node_info = {
        'gas_node_num': 7,
        'node_pressure_min': [75 / 1e5, 140 / 1e5, 100 / 1e5, 70 / 1e5, 150 / 1e5, 160 / 1e5, 100 / 1e5],
        'node_pressure_max': (np.array(
            [120 / 1e5, 170 / 1e5, 150 / 1e5, 120 / 1e5, 200 / 1e5, 240 / 1e5, 140 / 1e5]) * 1).tolist(),
        'gas_well_num': 2,
        'well_index': [6, 5],  # [0,0,4,5]
        'well_output_min': [0, 0],
        'well_output_max': [5300 * 28.3 / 1e6, 6000 * 28.3 / 1e6],
        'gas_load_num': 3,
        'load_index': [0, 2, 3],
        'gas_load_min': ((np.array(min_load)[:, 0:T]) / 1e6).tolist(),
        'gas_load_max': ((np.array(max_load)[:, 0:T]) / 1e6).tolist(),
        'gen_gas_num': 2,
        'gen_gas_index': [3, 0],
        'gen_gas_min': [0, 0],
        'gen_gas_max': [100, 100],
    ...
    gas_line_info = {
        ...
        'gas_line_active': [1, 5],
        'gas_flow_in_max': [100, 2000000, 100, 100, 100, 2000000],  # unused
        'gas_flow_out_max': [100, 2000000, 100, 100, 100, 2000000],  # unused
        'compressor_num': 2,
        'compressor_start_point': [1, 3],
        'compressor_end_point': [4, 1],
        'compressor_coefficient': [2.5, 2.5],
        'compressor_max_flow': [2000000 / 1e6, 2000000 / 1e6],
